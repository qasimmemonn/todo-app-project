      ...
      <header className="glass-nav sticky top-0 z-50 border-b border-slate-200/50">
        <div className="max-w-[1440px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="relative" ref={profileMenuRef}>
            <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="flex items-center gap-3 p-1 px-2 rounded-full hover:bg-slate-100 transition-all border border-transparent hover:border-slate-200">
      ...
      <div className="flex-1 flex max-w-[1440px] mx-auto w-full px-4 sm:px-6 py-8 gap-8">
        ...
        <main className="flex-1 min-w-0">
          <div className="mb-10 animate-fade-in">
            <h2 className="text-3xl font-black text-slate-900 leading-tight">Hello, {user.name?.split(' ')[0] || 'User'}! 👋</h2>
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 animate-fade-in">
          ...
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white mb-10 shadow-2xl animate-fade-in">
            <h3 className="text-xl font-black tracking-tight mb-6">New Goal</h3>
            <div className="space-y-4">
              <div className="relative">
                <input type="text" placeholder="Task title..." value={newTitle} onChange={(e) => setNewTitle(e.target.value)} className="w-full px-6 py-4 bg-white/10 border border-white/5 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white/20 outline-none transition-all font-bold text-white placeholder-white/30" />
                <button onClick={handleAiSuggest} disabled={!newTitle.trim() || aiLoading} className={`absolute right-3 top-3.5 p-1.5 rounded-xl transition-all ${!newTitle.trim() || aiLoading ? 'text-white/10' : 'text-indigo-400 hover:bg-white/10'}`}>
              ...
              <textarea placeholder="Details..." value={newDesc} onChange={(e) => setNewDesc(e.target.value)} className="w-full px-6 py-4 bg-white/10 border border-white/5 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white/20 outline-none transition-all text-sm text-white/70 resize-none h-24 placeholder-white/20" />
              <div className="flex justify-end">
                <button onClick={() => addTodo(newTitle, newDesc)} disabled={!newTitle.trim()} className="bg-white hover:bg-slate-100 disabled:bg-white/10 disabled:text-white/20 text-slate-900 font-black py-4 px-10 rounded-2xl transition-all uppercase tracking-widest text-xs">Create Task</button>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
            <div className="relative w-full sm:w-80 group">
               <input type="text" placeholder="Search keywords..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-medium focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 outline-none transition-all" />
